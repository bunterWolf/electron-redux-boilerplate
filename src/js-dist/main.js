...
function createNewBrowserWindow() {
  ...
  let browserWindow = new _electron.BrowserWindow({
    width: 800,
    height: 600,
    resizable: true,
    frame: false
...
_electron.ipcMain.on('global-favbar-changed', function (event) {
...
_electron.ipcMain.on('writeFile', function (event, path, content) {
  console.log('writeFile: ' + path);
  let pathObj = Utils.Path.createPathObj(path);

  _fs.fs.access(pathObj.dir, error => {
    if (!error) {
      _fs.fs.writeFileSync(path, content);
    } else {
      console.log(error);
    }
  });
...
_electron.ipcMain.on('global.newWindow', function (event, path) {
